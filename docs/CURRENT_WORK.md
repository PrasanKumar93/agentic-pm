...
- Review conflict quick action: review-state work items with a prior PR branch now show an `Update / Resolve Conflicts` button that submits a standard `request_changes` feedback turn. The feedback text is persisted in history and drives the same same-PR Codex/Cursor loop as typed reviewer feedback.
