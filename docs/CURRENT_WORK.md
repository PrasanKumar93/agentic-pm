...
- PR merge-conflict handoff gate: before creating or updating a GitHub PR, the worker now fetches the latest remote base and runs a non-mutating merge-tree check. Conflict-free branches continue to draft/update PRs; conflicting branches record `github.pr.merge_conflict`, block the work item, and do not move Linear to review-ready.
